...
| FR-F2 | Must | Blocked domains MUST resolve to NXDOMAIN. **As-built feed mix: `oisd_big` + `certpl` + `hagezi` (~463k domains).** ⚠️ **Open gap:** the original FR-F2 target — `hagezi Pro/Pro++`, `hagezi TIF`, `urlhaus` — do **NOT exist in adblock 4.5.6's feed catalog** (only generic `hagezi`, `oisd_*`, `certpl`, plus `doh_blocklist`, `stevenblack`, `adguard`, etc.). The three deployed feeds are the closest available match. (Do NOT add `doh_blocklist` without first confirming it does not list `cloudflare-dns.com`/`dns.google` — those are this router's own DoH upstreams; blocking them blackholes its resolver.) |
